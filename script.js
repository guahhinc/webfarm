...
let CROP_DATA = {
    ...
    'litchi': { name: 'Lychee', icon: '<img src="http://raw.githubusercontent.com/guahhinc/webfarm/main/litchi.png" alt="Lychee" style="height: 1.3em; width: 1.3em; object-fit: contain; vertical-align: middle;">', seedCost: 2000000, sellPrice: 3000000, growTime: 500000, weightRange: [0.9, 1.1], rarity: 'ultra_rare', stockChance: 0.5 },
...
const buildGrowingPlotMutations = (presetStatuses = []) => {
    ...
    // Limit to 2 natural mutations total to keep UI clean
    if (ss.length >= 2) return ss;

    const randRare = Math.random();
    // Roll for one mutation and stop
    if (randRare < SPECIAL_STATUSES.cosmic.chance) {
        ss.push('cosmic');
    } else if (randRare < SPECIAL_STATUSES.cosmic.chance + SPECIAL_STATUSES.rainbow.chance) {
        ss.push('rainbow');
    } else if (Math.random() < SPECIAL_STATUSES.pollinated.chance) {
        ss.push('pollinated');
    } else if (Math.random() < SPECIAL_STATUSES.golden.chance) {
        ss.push('golden');
    } else if (Math.random() < SPECIAL_STATUSES.silver.chance) {
        ss.push('silver');
    } else if (Math.random() < SPECIAL_STATUSES.bronze.chance) {
        ss.push('bronze');
    } else {
        const mapleChance = (gameState.currentWeather === 'autumn') ? 0.07 : SPECIAL_STATUSES.maple.chance;
        if (Math.random() < mapleChance) {
            ss.push('maple');
        } else {
            const twistedChance = (gameState.currentWeather === 'gale') ? 0.07 : SPECIAL_STATUSES.twisted.chance;
            if (Math.random() < twistedChance) {
                ss.push('twisted');
...
const computeFarmNetWorth = () => {
    ...
    v += (gameState.rebirthCount || 0) * 1e14; // Rebirth count remains the dominant factor
    return Math.floor(v);
};
...
const fetchLeaderboardJsonGet = async (paramsObj) => {
    ...
    try {
        ...
        const data = await parseWebAppJsonResponse(resp);
        if (data && data.ok) {
            if (data.version && data.version !== SAVE_CODE_VERSION) {
                showUpdatePopup();
            }
        ...
        if (data) logAppsScript(`GET fetch OK action=${action}`, data.ok);
...
const gameLoop = () => {
    ...
    gameState.plots.forEach(p => {
        if (p.state === 'growing' && p.growDuration) {
            ...
            // Random mutation chance during growth (approx 0.08% chance per 100ms tick)
            // Allows "further" mutations up to a total of 2
            if (p.specialStatuses.length < 2 && Math.random() < 0.0008) {
                const current = [...p.specialStatuses];
                const potential = buildGrowingPlotMutations(current);
                if (potential.length > current.length) {
                    p.specialStatuses = potential;
                    const newS = p.specialStatuses.find(s => !current.includes(s));
                    const statusInfo = SPECIAL_STATUSES[newS];
                    const cropInfo = CROP_DATA[p.crop];
                    if (statusInfo && cropInfo) {
                        showMutationNotification(statusInfo.icon, `${cropInfo.name} has mutated to ${statusInfo.name}`);
                    }
                    needsUpdate = true;
                }
            }
...
const pollGoogleSheetCommands = async () => {
    ...
    try {
        ...
        if (payload && payload.version && payload.version !== SAVE_CODE_VERSION) {
            showUpdatePopup();
        }
...
const setupEventListeners = () => {
    ...
    if (farmNameHeading) {
        farmNameHeading.addEventListener('click', () => {
            openSettingsPopup();
...
const UPDATE_LOG_ENTRIES = [
    ...
    },
    {
        ...
        changes: [
            'Added Breadfruit',
            'Added Citrus Caviar',
            'Added Mutations During Growth',
            'Can Now Click Name Above Farm To Change',
            'Big Background Code Changes',
            'Changed Some Pricing',
            'Moved Achievement Pop Up To Stop Blocking Top Bar',
            'Leaderboard Now Counts Rebirths In',
            'Bug Fixes',
        ],
    },
    {
...
const sortLeaderboardEntries = (entries) => {
    ...
    return [...filtered].sort((a, b) => {
        ...
        const nwA = Number(a.netWorth) || 0;
        const nwB = Number(b.netWorth) || 0;
        if (nwB !== nwA) return nwB - nwA;
        const mA = Number(a.money) || 0;
        const mB = Number(b.money) || 0;
        return mB - mA;
    });
};
...
const showLastSavedLeaderboard = () => {
    ...
    const sorted = sortLeaderboardEntries(cache.entries);
